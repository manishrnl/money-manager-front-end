...
const TransactionInfoCard = ({icon, title, date, amount, type, hideDeleteBtn, onDelete}) => {
    ...
    const getAmountStyles = () => type === 'income' ? 'bg-green-500 text-green-800' : 'bg-red-500 text-red-800';
    return (
        <div
            className="group relative flex items-center gap-4 mt-2 p-3 rounded-lg hover:bg-gray-100/60">
            <div
                className="w-12 h-12  flex items-center justify-center text-xl text-gray-800 bg-gray-100 rounded-lg ">
                {icon ? (
