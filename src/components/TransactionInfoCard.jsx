import {Layers2, Pencil, Trash2, TrendingUp} from "lucide-react";
import {addThousandsSeparator} from "../util/ThousandsSeparator.js";

const TransactionInfoCard = ({icon, title, date, amount, category, onDelete, onUpdate}) => {

    // FIX: Logic to handle URL icons from backend
    const renderIcon = () => {
        ...
        if (isLink) {
            return (
                <img
                    ...
                    onError={(e) => { e.target.src = "https://cdn-icons-png.flaticon.com/512/2454/2454282.png" }}
                />
            );
        }
            ...
            <div className="flex-1 flex items-center justify-between min-w-0">
                ...
                <div className="flex items-center gap-2">
                    <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={(e) => { e.stopPropagation(); onUpdate(); }} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg">
                            <Pencil size={16}/>
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="p-2 text-rose-500 hover:bg-rose-50 rounded-lg">
                            <Trash2 size={16}/>
                        </button>
                    </div>

                    <div className="flex items-center px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-xl border border-emerald-100">
                        <h6 className="flex items-center gap-1.5 text-xs font-black">
                            <span>+ ₹{addThousandsSeparator(amount)}</span>
                            <TrendingUp size={14}/>
                        </h6>
                    </div>
                </div>
