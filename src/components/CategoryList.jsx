...
// 1. Map your backend icon strings to Lucide Components
const ICON_MAP = {
    "directions_car": Car,
    "restaurant": Utensils,
    "payments": Briefcase, // Salary/Income
    "bolt": Zap,           // Utilities
            ...
            {categories.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    {categories.map((category) => {
                        // 2. Determine which icon to render
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="w-12 h-12 shrink-0 flex items-center justify-center rounded-xl bg-white shadow-sm border border-gray-100 group-hover:scale-110 transition-transform duration-300">
                                    <IconComponent
                                        size={22}
                                        className={category.type === 'INCOME' ? 'text-emerald-500' : 'text-indigo-500'}
                                    />
                                </div>
