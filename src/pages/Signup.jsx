...
const Signup = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!validateEmail(email)) {
            setIsLoading(false);
            return ShowPremiumToast("Please enter a valid email address", "error");
        }
            ...
            if (response.status === 201) {
                ...
                const successContent = (s) => (
                    <div className="flex flex-col gap-1 min-w-[250px]">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-bold text-sm uppercase tracking-wide text-emerald-800">Success</span>
                        </div>
                        <div className="ml-7 flex flex-col gap-1">
                            <p className="text-sm text-slate-600">Activation link sent to:</p>
                            <span className="font-mono font-bold text-emerald-700 text-xs break-all bg-emerald-50 p-1 rounded">{email}</span>
                            <p className="text-sm text-slate-600">Please activate your account and then try Logging in ...
                            </p>
                            <span className="text-[13px] text-gray-400 italic">Redirecting to Login page in {s} seconds ...</span>
                        </div>
                    </div>
                );

                const toastId = toast.success(successContent(secondsLeft), { duration: 15000 });
                ...
                const timer = setInterval(() => {
                    ...
                    if (secondsLeft <= 0) {
                        ...
                        toast.success(successContent(secondsLeft), { id: toastId });
                    }
                }, 1000);
            }
        } catch (error) {
            ...
            if (error.response?.status === 409) {
                ...
                const conflictContent = (s) => (
                    <div className="flex flex-col gap-1 min-w-[250px]">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-bold text-sm uppercase tracking-wide text-amber-800">Account Exists</span>
                        </div>
                        <p className="text-sm text-slate-600 ml-7">
                            This email is already registered. Login redirect in {s}s...
                        </p>
                    </div>
                );
                ...
                const timer = setInterval(() => {
                    ...
                    if (secondsLeft <= 0) {
                        ...
                        toast.error(conflictContent(secondsLeft), { id: toastId });
                    }
                }, 1000);
                return;
            }
    ...
    return (
        <div className="min-h-screen w-full relative flex items-center justify-center overflow-y-auto py-10">
            <img src={assets.finance} alt="Background" className="absolute inset-0 w-full h-full object-cover blur-xl scale-110 z-0" />
            ...
            <div className="relative z-10 bg-white/90 backdrop-blur-md rounded-2xl shadow-2xl w-[95%] sm:w-[85%] md:w-[65%] lg:w-[45%] p-10 border border-white/20">
                <h3 className="text-3xl font-bold text-slate-900 text-center mb-2">Create an Account</h3>
                <p className="text-sm text-slate-600 text-center mb-8">Start tracking your spending by joining with us</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <Input value={fullName} onChange={(e) => setFullName(e.target.value)} label="Full Name" placeholder="Manish Kumar" type="text" />
                    <Input value={email} onChange={(e) => setEmail(e.target.value)} label="Email Address" placeholder="sample@zohomail.in" type="email" />

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <Input value={password} onChange={(e) => setPassword(e.target.value)} label="Password" placeholder="*************" type="password" />
                        <Input value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} label="Confirm Password" placeholder="*************" type="password" />
                    </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mt-2 p-3 bg-slate-50 rounded-lg border border-slate-200">
                            {strengthCheck.map((check, index) => (
                                <div key={index} className="flex items-center gap-2 text-[11px]">
                                    <div className={`w-2 h-2 rounded-full ${check.met ? 'bg-green-500' : 'bg-slate-300'}`} />
                                    <span className={check.met ? 'text-green-700 font-medium' : 'text-slate-500'}>{check.label}</span>
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    <button
                        ...
                        {isLoading ? (
                            <><LoaderPinwheel className="animate-spin w-5 h-5" /><span>Signing Up...</span></>
                        ) : ("Sign up")}
                    </button>

                    <p className="text-center text-sm text-slate-600 mt-4">
                        Already have an account? <span className="text-blue-600 cursor-pointer hover:underline font-medium" onClick={() => navigate('/login')}>Login</span>
                    </p>
                </form>
            </div>
        </div>
